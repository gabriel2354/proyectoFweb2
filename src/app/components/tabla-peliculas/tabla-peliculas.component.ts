...
export class TablaPeliculasComponent implements OnInit {
  ...
  guardar(formulario: any) {
    // Si estamos editando, debemos usar el objeto película (que ya tiene el id)
    const nuevaPelicula: Pelicula = formulario.value;

    if (this.formularioPelicula) {
      // Si estamos editando, actualizamos la película
      if (this.pelicula.id) { // Verificamos que el id esté disponible
        this.peliculasService.actualizarPelicula(this.pelicula.id, nuevaPelicula).subscribe(
          () => {
            this.cargarPeliculas(); // Recargamos las películas después de editar
      ...
      this.peliculasService.agregarPelicula(nuevaPelicula).subscribe(
        () => {
